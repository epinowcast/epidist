/**
  * Compute the log probability mass function for a marginal model
  * with censoring
  *
  * This function is designed to be read into R where:
  * - 'family' is replaced with the target distribution
  *   (e.g., 'lognormal')
  * - 'dpars_A' is replaced with multiple distribution parameters
  *   in the format "real paramname1, real paramname2, ...".
  * - 'dpars_B' is replaced with the same parameters as dpars_A but
  *   reparameterised according to the brms parameterisation for
  *   Stan.
  *
  * @param y Real value of observed delay
  * @param dpars_A Distribution parameters (replaced via regex)
  * @param relative_obs_t Observation time relative to primary
  *   window start (D in primarycensored)
  ...
  * @param delay_min Minimum delay / left truncation point
  *   (L in primarycensored)
  * @param primary_params Array of parameters for primary
  *   distribution
  *
  * @return Log probability mass with censoring adjustment for
  *   marginal model
  */
  real marginal_family_lpmf(
    data int y, dpars_A, data real relative_obs_t,
