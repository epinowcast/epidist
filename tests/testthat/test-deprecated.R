test_that("predict_delay_parameters warns that it is deprecated", {
  skip_on_cran()
  skip_if_no_cmdstanr()
...
test_that("add_mean_sd warns that it is deprecated", {
  df <- data.frame(mu = 1.8, sigma = 0.5)
  class(df) <- c("lognormal_samples", class(df))
  expect_warning(add_mean_sd(df), class = "epidist_deprecated_warning")
})
...
test_that(
  "predict_delay_parameters works with NULL newdata and the latent and marginal lognormal model", # nolint: line_length_linter.
  {
    ...
    test_predictions <- function(fit, expected_rows = nrow(prep_obs)) {
      set.seed(1)
      pred <- suppressWarnings(predict_delay_parameters(fit))
...
test_that(
  "predict_delay_parameters works with the naive lognormal model",
  {
    ...
    set.seed(1)
    pred_naive <- suppressWarnings(predict_delay_parameters(fit_naive))
...
test_that("predict_delay_parameters accepts newdata arguments and prediction by sex recovers underlying parameters", { # nolint: line_length_linter.
  ...
  test_sex_predictions <- function(fit, prep = prep_obs_sex) {
    ...
    pred_sex <- suppressWarnings(predict_delay_parameters(fit, prep))
...
test_that("add_mean_sd.lognormal_samples works with simulated lognormal distribution parameter data", { # nolint: line_length_linter.
  ...
  class(df) <- c("lognormal_samples", class(df))
  x <- suppressWarnings(add_mean_sd(df))
...
test_that("add_mean_sd tells the user when there is no analytic solution", {
  df <- dplyr::tibble(mu = c(1, 2))
  class(df) <- c("exponential_samples", class(df))
  expect_message(
    suppressWarnings(add_mean_sd(df)),
    "no method available for this family"
  )
})
