test_that("epidist_family with default settings produces an object of the right class", {
...
test_that("epidist_family gives an error when passed inappropriate family input", {
  # nolint: line_length_linter.
  expect_error(epidist_family(prep_obs, family = 1))
  expect_error(epidist_family(prep_obs, family = list()))
})

test_that("the family argument in epidist_family passes as expected for brms and stats family objects, as well as strings", {
  # nolint: line_length_linter.
...
test_that("epidist_family contains the correct reparameterisations for lognormal (no change) and gamma (a change)", {
  # nolint: line_length_linter.
  family_lognormal <- epidist_family(prep_obs, family = "lognormal")
