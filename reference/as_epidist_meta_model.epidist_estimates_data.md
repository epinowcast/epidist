...
summary rows tilt the primary event by the `growth_rate` metadata of
their study, estimating it only where that is `NA` or has a
`growth_rate_sd`. Passing one is an error.
