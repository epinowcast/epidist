...
- trunc_design:
  ...
  adjust for censoring is exact for any `relative_obs_time`, `pwindow`
  and `swindow`. The weight used for the uniform single interval
  approximation is exact only for a narrow `pwindow`, and puts the
  implied mean about 3% high with a weekly primary window, a collection
  window of 28 days, a delay of mean 4.6 days and a growth rate of 0.2.
  See
