...
#'
#' @param data A `data.frame` containing line list data
#' @rdname as_cohort_model
#' @method as_cohort_model data.frame
#' @family cohort_model
#' @autoglobal
#' @export
as_cohort_model.data.frame <- function(data) {
  assert_cohort_model_input(data)
  class(data) <- c("epidist_cohort_model", class(data))
  data <- data |>
...
#' @family cohort_model
#' @export
is_cohort_model <- function(data) {
  inherits(data, "epidist_cohort_model")
}
