...
#' it is given as the family or as a model family built on it. For a `brms`
#' family they come from [.brms_family_fn()].
#'
...
.get_brms_fn <- function(prefix, family) {
  ...
  return(.brms_family_fn(prefix, .delay_family(family)$name))
}
...
#'
#' `primarycensored` 1.5.2 renamed `dprimary_args` to `primary_args`, because
#' the arguments reach both the primary density and its distribution function,
#' and kept the old name as a soft deprecation. A soft deprecation stays quiet
#' for calls from another package but warns while that package's tests run, so
#' the old name is only passed where the new one does not exist yet.
#'
#' @returns A length one character vector naming the argument.
#'
#' @keywords internal
.primary_args_name <- function() {
  installed <- package_version(getNamespaceVersion("primarycensored"))
  return(if (installed >= "1.5.2") "primary_args" else "dprimary_args")
}
