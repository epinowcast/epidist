...
biases, along with the data process it saw. The meta model supports the
...
are making alongside any results. See
...
#> ! "naive IQR" (row 3), "calendar stop" (row 6), and "midpoint" (row 11) report
...
Whether a study corrected for right truncation is the field reviews most
...
#> ! Setting 3 relative observation times (`relative_obs_time`) greater than 38
#>   (2x the maximum delay) to Inf.
#> ℹ This improves model efficiency by reducing the number of unique observation
#>   times in the data.
#> ℹ The impact on model accuracy should be negligible because these relative
...
As a reference, we fit the marginal model
([`vignette("epidist")`](https://epidist.epinowcast.org/articles/epidist.md))
...
whose metadata we have got wrong.
...
link. We assume that studies may differ by about a quarter on the log
scale, so both between study standard deviations get `normal(0, 0.25)`.
The phase coefficients get `normal(0, 0.5)`, so that two studies cannot
move the contrast without limit.
