...
#> Running MCMC with 2 parallel chains...
#> Chain 1 finished in 146.5 seconds.
#> Chain 2 finished in 146.7 seconds.
#>
#> Both chains finished successfully.
#> Mean chain execution time: 146.6 seconds.
#> Total execution time: 146.8 seconds.
```
...
the delay distribution, under no censoring or truncation, we may use
...
function.
[`epidist_newdata()`](https://epidist.epinowcast.org/reference/epidist_newdata.md)
expands the variables we give it into a grid and adds the response and
observation process variables the model uses, which saves us knowing the
column names the marginal model expects. Its defaults are no censoring
and no truncation. The
[`tidybayes::add_epred_draws()`](https://mjskay.github.io/tidybayes/reference/add_predicted_draws.html)
function uses the
