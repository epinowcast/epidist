...
It needs the `flexsurv` package and the development version of `primarycensored`, which adds the distribution to its Stan functions, so the pull request is not merged until that reaches CRAN.
