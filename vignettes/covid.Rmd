...
```{r}
china_covid_data <- read.csv(here("data", "china_covid_data.csv"))
...
na_values
dup_ids <- china_covid_data$unique_pair_id[duplicated(china_covid_data$unique_pair_id)] # will figure out how to handle these later, they are legitimate data
dup_ids
...
# Remove the rows where infectee onset date is before infector
negatives <- obs_prep %>% filter_if(is.numeric, any_vars(. < 0))
obs_prep <- obs_prep %>% anti_join(negatives, by = colnames(obs_prep))
...
A summary of the model shows that males tend to have longer delays (the posterior mean of the sexMale) and greater delay variation (the posterior mean of sigma_sexMale is 0.09).
For the sexMale effect and sigma_sexMale effect the 95% credible interval includes zero.
...
**As this is a longer running model than the previous two models but still runs within 1 minute (at 1000 iterations as well as the default of 2000). It is possible that for real world use cases this may not be sufficient.**
A small number of transitions ended in divergence. Read more about divergences and diagnostics from the links in the References below.
...
## References {-}
https://www.science.org/doi/10.1126/science.abc9004
