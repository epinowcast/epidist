...
To get started we will introduce some of the key concepts in delay distribution estimation, and then simulate some data.
Using this simulated data we will then show how to use the `epidist` package to estimate these distributions using a simple model and a model that accounts for some of the common issues in delay distribution estimation.
...
```{r lognormal, fig.cap="(ref:lognormal)", class.source = 'fold-hide'}
...
A common example of this is when events are reported as daily aggregates.
While daily censoring is most common, `epidist` supports the primary and secondary events having other delay intervals.

```{r cens, fig.cap="(ref:cens)", class.source = 'fold-hide'}
obs_cens |>
...
(ref:trunc) right truncation of the secondary event times biases the observation process towards shorter delays.

```{r trunc, fig.cap="(ref:trunc)", class.source = 'fold-hide'}
obs_cens |>
  filter(case %% 50 == 0, case <= 500) |>
  mutate(
...
```{r obs-est, fig.cap="(ref:obs-est)", class.source = 'fold-hide'}
...
Before fitting a model, it's useful to visualise the data to understand the distribution of delays and the observation process.
We'll create a plot showing the primary event times (symptom onset), secondary event times (case notification), and the observation time.

```{r}
linelist_data |>
  as_epidist_aggregate_data() |>
  select(ptime_lwr, stime_lwr, n) |>
  pivot_longer(
    cols = c("ptime_lwr", "stime_lwr"),
    names_to = "type",
    values_to = "time"
  ) |>
  ggplot() +
  geom_point(aes(x = date, y = n, col = type)) +
  geom_point(aes(x = obs_time, y = delay))
```

Note that here we have used `as_epidist_aggregate_data()` to convert the data into an aggregate format in order to make plotting the data easier.
...
`epidist` provides a range of models for different settings.
 The simplest is the naive model which does not account for censoring or truncation.
 ...
 Here all this does is define a class so that subsequent functions know which model is being used but for more complex models their `as_epidist_<model>model()` functions will create variables required specifically by the model and potentially take additional input.
...
As we used a simple model with only an intercept (see `vignettes("ebola")` for some complex options) the `Intercept` term corresponds to the mean log of the lognormal and the `sigma_Intercept` term corresponds to the sd log of the lognormal.

Remember that we simulated the data with a meanlog of 1.6 and a log standard deviation of 0.5.
We see that we have recovered neither of these parameters well (we have used a log link for the log sd so the target value is ~-0.69) and that means that the resulting distribution we have estimated will not reflect the data well.
...
`epidist` provides a range of models that can account for biases in observed data. In most cases, we recommend using the marginal model.
...
Behind the scenes it uses a likelihood from the [`primarycensored` R package](primarycensored.epinowcast.org).
...
Compared to the naive fit we see good recovery of the true distribution parameters (remember these were 1.6 for the logmean and 0.5 (or ~-0.69 on te log scale)) for the log sd.
...
(ref:fitted-lognormal) The posterior draws from the marginal and naive models (coloured lines, 100 draws from each posterior) compared to the true underlying delay distribution (black line).
 Each coloured line represents one possible delay distribution based on the estimated model parameters.
 The naive model shows substantial bias whilst the marginal model better recovers the true distribution.

```{r fitted-lognormal, fig.cap="(ref:fitted-lognormal)", class.source = 'fold-hide'}
set.seed(123)
