...
First, we use the [Gillespie algorithm](https://en.wikipedia.org/wiki/Gillespie_algorithm) to generate infectious disease outbreak data (Figure \@ref(fig:outbreak)) from a stochastic compartmental model.
...
<div class="figure">
<img src="figures/epidist-outbreak-1.png" alt="(ref:outbreak)"  />
...
<div class="figure">
<img src="figures/epidist-delay-1.png" alt="(ref:delay)"  />
...
To do this we introduce a `outbreak_start_date` which is the date of the first infection, and use `simulate_dates()` to floor the event times to their reporting window and offset them from it.
Here we rename the columns to show that `epidist` does not require particular names.
...
`plot_events()` draws the event windows of a linelist (Figure \@ref(fig:linelist)).
It gives the same view as Figure \@ref(fig:trunc), for the data we will fit to.

(ref:linelist) The primary and secondary event windows of each observed case, ordered by the date of symptom onset. The dashed line is the observation date, after which no secondary event is seen.
...
`type = "delay"` draws the delay distribution the parameters imply, one line per draw when `ndraws` is given.
