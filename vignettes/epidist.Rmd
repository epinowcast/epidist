...
One way to do this is to use the `add_delay_parameter_draws()` function to extract the posterior samples and then plot them.
The `add_summaries()` function then adds the mean and standard deviation of the delay distribution so that it is easier to understand.
