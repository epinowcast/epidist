...
::: {.alert .alert-primary}
This vignette fits with the `cmdstanr` backend where CmdStan is installed and with the default `rstan` backend otherwise.
We generally recommend the `cmdstanr` backend for faster sampling and additional features.
It can be set for a single fit using `backend = "cmdstanr"`, or for a session using `options(brms.backend = "cmdstanr")`, after following the installing CmdStan instructions in the README.
:::
...
`delay_parameter_draws()`, `add_summaries()` and `epidist_strata()` are the three steps it wraps, and each is available on its own.
