...
For example, samples of the fitted lognormal `mu` and `sigma` parameter can be extracted using:

```{r}
draws <- delay_samples(fit)
```
...
ggplot() +
  ...
  geom_function(
    ...
    aes(x = x), fun = dlnorm,
    args = list(
      meanlog = mean(draws$mu),
      sdlog = mean(draws$sigma)
    ),
    col = "#CC79A7"
  ) +
  labs(
