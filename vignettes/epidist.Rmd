...
```{r}
naive_fit <- epidist(
  naive_data, chains = 2, cores = 2, refresh = ifelse(interactive(), 250, 0)
)
```
...
marginal_fit <- epidist(
  data = marginal_data, chains = 2, cores = 2,
...
```{r}
pred <- predict_delay_parameters(naive_fit)
```
