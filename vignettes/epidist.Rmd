...
obs_cens_trunc_samp <- slice_sample(
  obs_cens_trunc, n = sample_size, replace = FALSE
)
```
...
```{r}
naive_fit <- epidist(
  naive_data, chains = 4, cores = 2, refresh = ifelse(interactive(), 250, 0)
)
```
...
predicted_parameters |>
  ...
  ggplot() +
  geom_density(
    aes(x = value, fill = model, group = model), alpha = 0.5
  ) +
  geom_vline(
    data = true_params,
...
predicted_pmfs |>
  ...
  ggplot() +
  geom_line(
    aes(x = x, y = y, col = model, group = draw), alpha = 0.05, linewidth = 1
  ) +
  geom_line(
