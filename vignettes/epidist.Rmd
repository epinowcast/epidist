...
```{r}
obs_cens_trunc <- filter(obs_cens, .data$sdate_upr <= .data$obs_date)
```
...
Our data is now very nearly what we would observe in practice but as a final step we rename the columns to show that `epidist` does not require particular names.
...
# Visualising the data {#vis}

`simulate_dates()` names its columns as `as_epidist_linelist_data()` expects, so the complete simulated data converts without any arguments.
Comparing it with `linelist_data` shows what the observation process removed.

```{r}
linelist_all <- as_epidist_linelist_data(obs_cens)
```
...
plot_events(filter(linelist_all, .data$case <= 500))
```
...
The same view of `linelist_data` shows the data we will fit to (Figure \@ref(fig:linelist)).
...
```{r}
plot_data <- bind_rows(
  "Censored retrospective data" = obs_cens,
  "Censored, truncated,\nsampled data" = obs_cens_trunc_samp,
  .id = "type"
) |>
