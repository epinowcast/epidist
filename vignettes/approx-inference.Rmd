...
NUTS is an example of a broader class of Markov chain Monte Carlo (MCMC) methods which work by simulating from a Markov chain with the targetted posterior distribution as stationary distribution.
...
A drawback of MCMC methods like NUTS, is that they can be quite computational intensive, especially for complex models or large data.
...
Here we describe three alternative approximate Bayesian inference algorithms that are available to use in `brms` and therefore `epidist`.
...
Automatic differentiation variational inference [ADVI; @kucukelbir2017advi] is a variety of variational inference algorithm.
