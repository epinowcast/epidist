...
fit_hmc <- epidist(
  data = data, algorithm = "sampling", backend = "cmdstanr",
  chains = 1, iter = 5
)
```
...
```{r results='hide'}
t <- proc.time()
fit_hmc <- update(fit_hmc, chains = 4, iter = 2000)
...
t <- proc.time()
fit_laplace <- update(
  fit_hmc, algorithm = "laplace", draws = 4000
)
time_laplace <- proc.time() - t

t <- proc.time()
fit_advi <- update(
  fit_hmc, algorithm = "meanfield", draws = 4000
)
...
fit_pathfinder <- update(
  fit_hmc, algorithm = "pathfinder", draws = 4000,
  chains = 1
)
