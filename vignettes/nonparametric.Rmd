...
The logit hazards follow a random walk, so neighbouring bins have similar hazards.
The `mu` parameter shifts the logit hazard of every bin, so a covariate in its formula gives each stratum its own delay.
...
#> Total execution time: 118.1 seconds.
...
#>   <chr>          <chr>     <dbl> <dbl> <dbl>
#> 1 Non-parametric younger    3.76  2.71  7
#> 2 Non-parametric older      6.91  4.73 14
#> 3 Lognormal      younger    3.96  3.48  7.83
#> 4 Lognormal      older      7.30  7.28 15.0
...
#> 1 naive    mean      5.91    NA   150 NA          1       1                15
#> 2 naive    sd        3.76    NA   150 NA          1       1                15
#> 3 quantile quantile  3       NA   100  0.25       1       1                20
#> 4 quantile quantile  5       NA   100  0.5        1       1                20
#> 5 quantile quantile 10       NA   100  0.75       1       1                20
#> 6 window   mean      7.02    NA   200 NA          1       1                25
#> 7 window   sd        4.34    NA   200 NA          1       1                25
...
#> Running MCMC with 2 parallel chains...
#>
#> Chain 1 finished in 389.2 seconds.
#> Chain 2 finished in 394.7 seconds.
#>
#> Both chains finished successfully.
#> Mean chain execution time: 391.9 seconds.
#> Total execution time: 394.9 seconds.
```
...
It lets strata with little data borrow the shape of the delay from the others, but it cannot give strata delays of different shapes.
* The random walk on the logit hazard smooths the fitted delay.
`hazard_model = "re"` treats the hazards as independent around `mu` instead.
* Each bin is a parameter, so a long delay on daily bins is slower to fit than a parametric family.
