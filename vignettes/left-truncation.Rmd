...
Only delays above $L$ can enter the sample, so the observed delays follow the conditional distribution
$$
F(t \mid T > L) = \frac{F(t) - F(L)}{1 - F(L)}, \qquad t > L .
$$
Fitting $F$ directly to such a sample biases the estimate.
The model has to explain the missing short delays by shifting the whole distribution.

With right truncation at $D$ as well, the likelihood renormalises over $[L, D]$
$$
\frac{F(t) - F(L)}{F(D) - F(L)} .
$$

The `delay_min` argument of `as_epidist_marginal_model()` sets $L$.
It is passed to the [`primarycensored`](https://primarycensored.epinowcast.org/) likelihood as its `L` argument.
...
obs <- simulate_gillespie(r = 0.2, seed = 101) |>
  simulate_secondary(distspec::LogNormal(true_meanlog, true_sdlog))
```
...
`simulate_dates()` adds daily reporting windows and returns date.
...
<div class="figure">
<img src="figures/left-truncation-hist-1.png" alt="Observed delays after left truncation. The dashed line marks the minimum delay."  />
<p class="caption">(\#fig:hist)Observed delays after left truncation. The dashed line marks the minimum delay.</p>
</div>
...
A `delay_min` column already in the data is picked up without the argument.
...
<div class="figure">
<img src="figures/left-truncation-params-plot-1.png" alt="Posterior draws of the lognormal parameters, and of the natural scale mean and standard deviation. Dashed lines are the simulation values. The unadjusted model is biased away from them."  />
<p class="caption">(\#fig:params-plot)Posterior draws of the lognormal parameters, and of the natural scale mean and standard deviation. Dashed lines are the simulation values. The unadjusted model is biased away from them.</p>
</div>
...
<div class="figure">
<img src="figures/left-truncation-pdf-plot-1.png" alt="Estimated delay distributions against the true lognormal density (black line)."  />
<p class="caption">(\#fig:pdf-plot)Estimated delay distributions against the true lognormal density (black line).</p>
</div>
