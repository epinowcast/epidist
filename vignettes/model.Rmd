...
This is primarily done via the function `latent_truncation_censoring_adjusted_delay`, which implements a Bayesian latent variable model.
In Section \@ref(maths), we explain the model's mathematical structure.
In Section \@ref(implementation), we outline how the model is implemented, making use of the `brms` R package for fitting Bayesian hierarchical models [@brms].
...
Additionally, due to the model's implementation using `brms`, readers will be able to make modifications to the model's structure to suit particular analysis needs.
...
This section condenses @park2024estimating, which may be referred to for further information.
...
To explain how the model is implemented in `epidist`, we begin by simulating suitable data:
...
The function `latent_truncation_censoring_adjusted_delay` has eight arguments^[As well as the capacity to pass additional arguments to `brms` anonymously via `...`].
...
Next, this is the function used to fit the model.
What other things could it be?
Examples in `epidist-paper` here.

```{r}
fn <- brms::brm
```

Defines a custom `brms` family.
See package vignette for more about this.
...
These are additional bits of Stan code passed to `brms`.
(Adam to understand.)
