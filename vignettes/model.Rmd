...
When the two censoring windows overlap the delay must still be non-negative, so the primary event has to precede the sampled secondary event.
The offset is then bounded by the sampled secondary offset instead of by the window width,
$$
p_i = s_i\, \tilde{p}_i .
$$
...
$$
and Stan does not add it for a transformation written this way.
The log density of these observations needs the corresponding term,
$$
\log \mathcal{L}_i \;\longmapsto\; \log \mathcal{L}_i + \log s_i .
$$
The ordinary case needs no such term because $w_{P, i}$ is data, so its Jacobian is a constant that cancels.
The primary event density has to be used unnormalised here, since a constant normalising it over $[0, s_i]$ would itself depend on $s_i$.

With the adjustment, integrating the augmented density over $\tilde{p}_i$ and $\tilde{s}_i$ recovers the double interval censored likelihood of Section \@ref(interval-censoring), up to a constant that does not depend on $\theta$.
The latent and marginal models then target the same likelihood.
Without it the implied primary event density is proportional to $1 / s_i$ rather than flat across the window.

Only observations whose windows overlap are affected.
Under daily censoring these are exactly the delays observed as zero, so the bias is small when windows are short relative to the delay and grows as they widen.

The latent model formulation follows @ward2022transmission.
The missing adjustment was identified in a re-analysis of the 2012 Isiro Bundibugyo virus disease line list [@bdbvlinelist], and is derived in @brand2026scalable.
