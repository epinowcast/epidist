...
This is primarily done via the function `latent_truncation_censoring_adjusted_delay`, which implements a Bayesian hierarchical model.
In Section \@ref(maths), we explain the model's mathematical structure.
In Section \@ref(implementation), we outline how the model is implemented, making use of the `brms` R package for fitting Bayesian hierarchical models.
After reading this vignette, we hope you are able to confidently use the `latent_truncation_censoring_adjusted_delay` function, as well as make modifications to the model's structure to suit your needs.
...
To introduce the structure of the model, we require some notation.
This section condenses the writing of @park2024estimating, which may be referred to for further information.
