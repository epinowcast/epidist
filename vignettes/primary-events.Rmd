...
fit_growing <- epidist(
  ...
  prior = prior(normal(0, 0.5), class = "Intercept", dpar = "pgrowth"),
  chains = 2, cores = 2, refresh = 0, silent = 2
)
```

The tilted model recovers the growth rate used to simulate.
...
The estimated rates track the values used to simulate.
...
The rate is also only weakly identified from delays alone at daily resolution.
In the single rate fit above its interval is wide, so treat a point estimate with caution.

The growth rate is estimated here rather than fixed, so its uncertainty propagates into the delay estimate.
Fixing it is also possible, by supplying a constant prior.
