...
prior_new <- brms::prior("skew_normal(1.5, 0.5, 3)", class = "Intercept") +
    brms::prior("normal(-1, 0.5)", class = "Intercept", dpar = "sigma")

# This is annoying and deserves an issue
prior_new[is.na(prior_new)] <- ""

fit_ppc <- epidist(
  data = data, family = brms::lognormal(), sample_prior = "only", seed = 1, prior = prior_new
)
```
