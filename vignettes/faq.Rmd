...
For example, for the `brms::lognormal()` latent model, we suggest the following prior distributions for the `brms` `mu` and `sigma` intercept parameters:
...
epidist_formula <- epidist_formula(
  data,
  family = epidist_family, formula = mu ~ 1
)
...
For example, for the `latent_model` model, currently methods are implemented for the [lognormal](https://github.com/epinowcast/epidist/blob/main/R/latent_lognormal.R) and [gamma](https://github.com/epinowcast/epidist/blob/main/R/latent_gamma.R) families.
