...
The output of a call to `epidist` is compatible with typical Stan workflows.
We recommend use of the [`bayesplot`](https://mc-stan.org/bayesplot/) package for sampling diagnostic plots.
...
<div class="figure">
<img src="figures/faq-unnamed-chunk-4-1.png" alt="plot of chunk unnamed-chunk-4"  />
<p class="caption">(\#fig:unnamed-chunk-4)plot of chunk unnamed-chunk-4</p>
</div>
...
`brms::pp_check()` is a convenient function for this, but it does not automatically handle the aggregated/weighted data structure used by `epidist` models.
Because `brms::pp_check()` ignores the `weights` argument when plotting observed data, passing aggregated data directly will result in an incorrect comparison between observed and predicted distributions.

To use `pp_check()` correctly with aggregated data, you must first expand the data to an individual-level format (one row per observation) using `tidyr::uncount()`.
Then, pass this expanded data to the `newdata` argument of `pp_check()`.
...
<div class="figure">
<img src="figures/faq-unnamed-chunk-6-1.png" alt="plot of chunk unnamed-chunk-6"  />
<p class="caption">(\#fig:unnamed-chunk-6)plot of chunk unnamed-chunk-6</p>
</div>

For more advanced custom visualizations, you can also use `tidybayes::add_predicted_draws()` as demonstrated in the ["Advanced features with Ebola data"](https://epidist.epinowcast.org/articles/ebola.html) vignette.

## What is an observation in the meta model?

`as_epidist_meta_model()` fits the summaries a study computed from the same delays jointly, so one row of the model data is a group of summaries rather than a single reported value.
This is the same situation as aggregated data, where one row stands for many individual delays.
`brms::log_lik()` and `loo::loo()` work at the row level, so their values are per group, and `pp_check()` needs the same care as described above.
Predictions for summary rows are the reported summary itself rather than a delay, so restrict `pp_check()` to the individual level rows of a mixed fit.
...
[`brms`](https://paulbuerkner.com/brms/) provides default priors for all parameters.
...
<div class="figure">
<img src="figures/faq-unnamed-chunk-9-1.png" alt="plot of chunk unnamed-chunk-9"  />
<p class="caption">(\#fig:unnamed-chunk-9)plot of chunk unnamed-chunk-9</p>
</div>
...
We recommend use of the [`priorsense`](https://github.com/stan-dev/priorsense) package [@kallioinen2024detecting] to check how sensitive the posterior distribution is to perturbations of the prior distribution and likelihood using power-scaling analysis:
...
<div class="figure">
<img src="figures/faq-unnamed-chunk-10-1.png" alt="plot of chunk unnamed-chunk-10"  />
<p class="caption">(\#fig:unnamed-chunk-10)plot of chunk unnamed-chunk-10</p>
</div>

# What do the parameters in my model output correspond to?

The `epidist` package uses [`brms`](https://paulbuerkner.com/brms/) to fit models.
...
This parameter does not necessarily correspond to the mean, hence the provision of a function `add_summaries()` within `epidist` to add columns containing the natural scale mean, standard deviation and quantiles to a `data.frame` of draws.
...
However this is tricky to do, and so where possible we recommend using the [`tidybayes`](https://mjskay.github.io/tidybayes/) package.
...
<div class="figure">
<img src="figures/faq-unnamed-chunk-11-1.png" alt="plot of chunk unnamed-chunk-11"  />
<p class="caption">(\#fig:unnamed-chunk-11)plot of chunk unnamed-chunk-11</p>
</div>
