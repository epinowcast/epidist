...
For the time being, we filter the data to only complete cases (i.e. rows of the data which have no missing values^[An extension is neededto allow for missing data in the model - please open issue if this would be useful for you.]).
...
To simulate being in the middle of an outbreak we will filter the data to only include cases up to the 1st of January 2015.
...
```{r}
obs_cens <- obs_cens |>
  filter(date_of_sample_tested <= as.Date("2015-01-01"))
```
...
**As this is a longer running model we have reduced the number of iterations but for real world use cases this may not be sufficient.**
