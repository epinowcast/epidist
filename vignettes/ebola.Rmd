...
The 2014-2016 outbreak of Ebola in West Africa was the largest outbreak in the disease's history, and resulted in over 28,600 infections and 11,325 deaths [@who_ebola_2014_2016].
...
We start by fitting an intercept-only model, in which we fit a lognormal distribution to the overall data. 
...
To do this, we set `formula` to `brms::bf(mu ~ 1, sigma ~ 1)` to place an intercept only model on the `mu` and `sigma` parameters of the lognormal distribution.
This distribution is specified using `family = brms::lognormal()`.
...
To fit a model which varies the parameters of the fitted lognormal distribution, `mu` and `sigma`, by sex we alter the `formula` specification to include fixed effects for sex as follows:
...
No stratification (Figure \@ref(fig:pmf)):
...
(ref:pmf) Figure caption.

```{r pmf, fig.cap="(ref:pmf)"}
...
Stratification by sex, omitting delays greater than 30 (Figure \@ref(fig:pmf-sex)):
...
(ref:pmf-sex) Figure caption.

```{r pmf-sex, fig.cap="(ref:pmf-sex)"}
draws_sex_pmf |>
  mutate(
    sex = case_when(
      sex == "F" ~ "Female",
...
Stratification by sex-district (Figure \@ref(fig:pmf-sex-district)):
...
(ref:pmf-sex-district) Figure caption.

```{r pmf-sex-district, fig.cap="(ref:pmf-sex-district)", fig.height = 9}
...
No stratification (Figure \@ref(fig:pdf)):
...
(ref:pdf) Figure caption.

```{r pdf, fig.cap="(ref:pdf)"}
...
Stratification by sex (Figure \@ref(fig:pdf-sex)):
...
(ref:pdf-sex) Figure caption.

```{r pdf-sex, fig.cap="(ref:pdf-sex)"}
draws_sex_pmf |>
  mutate(
    sex = case_when(
      sex == "F" ~ "Female",
...
Stratification by sex-district (Figure \@ref(fig:pdf-sex-district)):
...
(ref:pdf-sex-district) Figure caption.

```{r pdf-sex-district, fig.cap="(ref:pdf-sex-district)", fig.height = 9}
