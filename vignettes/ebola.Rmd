...
To prepare the data for use with the latent model, we set `obs_cens` to be an `epidist_linelist` object, then use the function `as_latent_model()`:
...
obs_prep <- as_latent_model(linelist)
head(obs_prep)
```

Now we are ready to fit the latent model.
