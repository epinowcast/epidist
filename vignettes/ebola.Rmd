...
<details><summary>Click to expand for code to prepare data for post-processing</summary>
...
<details><summary>Click to expand for code to prepare expectation plot</summary>
...
p_expec_naive <- expectation_draws |>
  ggplot(aes(x = .epred)) +
  stat_halfeye() +
...
(ref:expec-naive) Posterior prediction of the delay expectation for the naive (no stratification) model.

```{r expec-naive, fig.cap="(ref:expec-naive)"}
p_expec_naive
```

We can do the same with the sex-stratified model (Figure \@ref(fig:expec-sex)):

<details><summary>Click to expand for code to prepare sex-stratified expectation plot</summary>
...
p_expec_sex <- expectation_draws_sex |>
...
(ref:expec-sex) Posterior prediction of the delay expectation for the sex-stratified model.

```{r expec-sex, fig.cap="(ref:expec-sex)"}
p_expec_sex
```

And if we want to see the posterior expectations for all combinations of sex and district, we would do (Figure \@ref(fig:expec-sex-district)):

<details><summary>Click to expand for code to prepare sex-district expectation plot</summary>
...
p_expec_sex_district <- expectation_draws_sex_district |>
...
(ref:expec-sex-district) Posterior prediction of the delay expectation for the sex-district stratified model.

```{r expec-sex-district, fig.cap="(ref:expec-sex-district)"}
p_expec_sex_district
```
...
p_pmf <- pmf_base_figure / pmf_sex_figure / pmf_sex_district_figure +
...
p_pdf <- pdf_base_figure / pdf_sex_figure / pdf_sex_district_figure +
  plot_layout(heights = c(1, 1.5, 5.5))
```
