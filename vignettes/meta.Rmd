...
description: "Combining summary estimates from the literature with individual level data"
output:
...
The meta model lets us fit directly to these published summaries, forward modelling what the study's own estimation procedure would have converged to, and combine that evidence with any individual level data we do have.
...
* **How the study adjusted for censoring** (`cens_adjusted`), one of `0` if it took integer date differences and summarised them directly (the most common case), `1` if it used a method that targets the continuous delay, such as a latent variable or marginal likelihood, or `2` if it adjusted only the secondary interval assuming a uniform delay within it.
* **Whether it adjusted for right truncation** (`trunc_adjusted`), and if not, **the observation time** (`relative_obs_time`), the truncation point on the delay scale.
...
fit_reference <- suppressMessages(epidist(
...
We treat a study as not adjusted for right truncation, with an observation time of three times its reported value, only if it was conducted during the outbreak (`method_moment_value` of `"Start outbreak"` or `"Mid outbreak"`); all other studies are treated as unaffected by truncation.
epireview does not record the true observation cutoff, so this is a rough, explicitly stated, placeholder rather than a reported value.
...
row_to_estimates <- function(
  study, n, trunc_adjusted, relative_obs_time, cens_adjusted, max_delay,
  ...
  base <- tibble(
    ...
    relative_obs_time = relative_obs_time, cens_adjusted = cens_adjusted,
    max_delay = max_delay
  )
...
fit_ebola <- suppressMessages(epidist(
  data = ebola_meta,
...
The posterior draws sit broadly within the range of the reported fits, which cover separate outbreaks in one country, so some spread between them is expected.
The pooled estimate is not driven by those alone, as it also uses the remaining `r nrow(onset_to_death) - nrow(reported_gamma)` summaries from other countries and outbreaks.
