...
#> ! The smallest quantile reported by "naive IQR", "calendar stop", and
#>   "midpoint" sits within ten censoring windows of the smallest delay these
#>   studies counted, so the discrete grid barely resolves the delay. A reported
#>   quantile is rounded to that grid, which can bias the fit by tens of percent
#>   and does not shrink as `n` grows. Check that `swindow` is the resolution the
#>   study worked at, and fit a reported mean and standard deviation in preference
#>   where the study gives them.
#> ! "calendar stop" reports several quantiles of integer day delays from more
#>   than 100 delays. The joint likelihood of such a quantile set is overconfident
#>   once the binomial spread of the crossing point of the empirical distribution
#>   function is narrower than a day, which it is here, so this study will be
#>   weighted too heavily. Fit a reported mean and standard deviation instead
...
#> Intercept           1.78      0.02     1.75     1.81 1.00      835      603
#> sigma_Intercept    -0.69      0.03    -0.74    -0.63 1.00      645      445
#>
...
`delay_parameter_draws()` puts both parameters back on the scale the simulation used.
...
#> Regression Coefficients:
#>                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           1.72      0.01     1.69     1.75 1.00      811      654
#> sigma_Intercept    -0.73      0.03    -0.78    -0.67 1.00      835      702
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
#> Regression Coefficients:
#>                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           1.78      0.02     1.75     1.81 1.01      854      725
#> sigma_Intercept    -0.69      0.03    -0.74    -0.64 1.00      822      665
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
#>   reporting) for every study.
#> ! The grid cutoff `max_delay` for "Aylward 2014" is short relative to the tail
#>   of the delay this study reported. A lognormal matched to the reported
#>   summaries has more than 2% of its second moment beyond the cutoff, so the
#>   implied standard deviation will be biased downwards. Increase `max_delay` for
#>   this study.
#> ! The smallest quantile reported by "Nanclares 2016", "Li 2016 (a)", "Khan
#>   1999", "Camacho 2014", "Baron 1983", "Bah 2015", "Miglietta 2019", and "Nsio
#>   2019" sits within ten censoring windows of the smallest delay these studies
#>   counted, so the discrete grid barely resolves the delay. A reported quantile
#>   is rounded to that grid, which can bias the fit by tens of percent and does
#>   not shrink as `n` grows. Check that `swindow` is the resolution the study
#>   worked at, and fit a reported mean and standard deviation in preference where
#>   the study gives them.
#> ! The standard deviation reported by "Muoghalu 2017", "Rosello 2015 (2)",
#>   "Maganga 2014", and "Folarin 2016" has a relative standard error above 0.25
#>   at the kurtosis its mean and standard deviation imply under a lognormal
#>   delay. The normal sampling likelihood of a reported standard deviation, and
#>   the joint likelihood of a mean and standard deviation pair, cannot be trusted
#>   for such a heavy tail at this sample size. Where the study reports quantiles
#>   inside the body of the distribution, those are safer.
```
...
We fit a Gamma, with a `(1 | study)` term on both the mean and the shape to allow for genuine differences between studies in either (see the note on plug in standard errors in `?as_epidist_meta_model`).
The meta model puts no restriction on the `brms` formula, so a study term on the shape is written as it would be for any distributional parameter.
The meta model centres the prior for the intercept on the log of the median mean the studies report, and puts a half normal prior with a standard deviation of 0.25 on the between study standard deviation of each parameter, which on the log scale allows studies to differ by about a quarter.
Both are worth seeing before fitting.
...
epidist_prior(
  ebola_meta,
  family = epidist_family(ebola_meta, family = Gamma(link = "log")),
  formula = epidist_formula(
    ebola_meta, epidist_family(ebola_meta, family = Gamma(link = "log")),
    formula = ebola_formula
  ),
  prior = NULL
)
#>            prior     class coef group resp  dpar nlpar   lb   ub source  tag
#>  normal(2.23, 1) Intercept                             <NA> <NA>  model <NA>
#>  normal(0, 0.25)        sd                             <NA> <NA>  model <NA>
#>  normal(0, 0.25)        sd                 shape       <NA> <NA>  model <NA>
```


``` r
# With a study term on both parameters the default initial range of brms
# lets a study's shape start far outside where the model is finite, so the
# chains start closer in.
fit_ebola <- epidist(
  data = ebola_meta,
  formula = ebola_formula,
...
#>                     Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sd(Intercept)           0.14      0.03     0.08     0.21 1.02      297      530
#> sd(shape_Intercept)     0.52      0.10     0.36     0.74 1.02      417      776
#>
#> Regression Coefficients:
#>                     Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept               2.24      0.07     2.11     2.37 1.00      566      656
#> shape_Intercept         1.32      0.16     1.02     1.62 1.00      482      662
#> phaseduringoutbreak     0.07      0.13    -0.20     0.34 1.00      770      722
#> phaseunrecorded        -0.02      0.08    -0.18     0.13 1.00      568      604
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
``` r
# One individual level row per phase, with no censoring and no truncation.
# The columns of `newdata` come back with the draws, so `phase` is already
# on the result.
...
#>   parameter median  q025  q975
#>   <chr>      <dbl> <dbl> <dbl>
#> 1 shape       3.75  2.78  5.08
#> 2 scale       2.50  1.79  3.47
#> 3 mean        9.35  8.22 10.7
#> 4 sd          4.84  3.94  5.99
```
...
#>  during outbreak - post outbreak    0.627 -1.84   3.60
#>  unrecorded - post outbreak        -0.141 -1.71   1.18
#>
#> Term: phase
#> Type: response
```
...
ebola_params |>
  ...
  transmute(
    study = .data$article_label,
    delay = .data$parameter_type_short,
    family = .data$distribution_type,
    shape = .data$distribution_par1_value,
    scale = .data$distribution_par2_value,
    # The mean the reported parameters imply, to compare with the mean the
    # same record reports as its estimate.
    implied_mean = ifelse(
      .data$distribution_type == "Gamma",
      .data$shape * .data$scale,
      .data$scale * gamma(1 + 1 / .data$shape)
    ),
    reported_mean = .data$parameter_value,
    n = .data$population_sample_size
  )
#> # A tibble: 4 × 8
#>   study             delay    family shape scale implied_mean reported_mean     n
#>   <chr>             <chr>    <chr>  <dbl> <dbl>        <dbl>         <dbl> <int>
#> 1 Fallah 2015 (1)   incubat… Gamma   2.86  3.51        10.0           NA      NA
#> 2 Fallah 2015 (2)   incubat… Gamma   2.60  3.35         8.71          NA      NA
#> 3 Marziano 2023 (1) serial_… Weibu…  1.76 10.1          9.03          12      24
#> 4 Marziano 2023 (2) serial_… Weibu…  1.63 12.0         10.7           11.7    12
```
...
#> ! The smallest quantile reported by "Marziano 2023 (2)" sits within ten
#>   censoring windows of the smallest delay this study counted, so the discrete
#>   grid barely resolves the delay. A reported quantile is rounded to that grid,
#>   which can bias the fit by tens of percent and does not shrink as `n` grows.
#>   Check that `swindow` is the resolution the study worked at, and fit a
#>   reported mean and standard deviation in preference where the study gives
#>   them.
#> ! The standard deviation reported by "Marziano 2023 (2)" has a relative
#>   standard error above 0.25 at the kurtosis its mean and standard deviation
#>   imply under a lognormal delay. The normal sampling likelihood of a reported
#>   standard deviation, and the joint likelihood of a mean and standard deviation
#>   pair, cannot be trusted for such a heavy tail at this sample size. Where the
#>   study reports quantiles inside the body of the distribution, those are safer.
#> # A tibble: 5 × 16
#>   study          type  value    se     n     p pwindow swindow relative_obs_time
...
The `implied_mean` and `reported_mean` columns above are worth comparing before using either.
`Marziano 2023 (1)` disagrees by 25% of its reported mean, and the 9.03 days its Weibull implies falls below the 10 to 14.2 interval it reports around that mean of 12.
`Marziano 2023 (2)` disagrees by 8%, and its implied mean sits inside a reported interval of 8.2 to 15.8, which is wide enough that this is a weak test.
A record where the two disagree is telling you that the summary and the fitted distribution came from different calculations, and you need to know which one you want before fitting either.
We use `Marziano 2023 (2)`, the closer of the two.
...
Reported parameters without their standard errors sit between the third and fourth rungs of the ordering set out at the start of this vignette.
They pin down the shape of the distribution the study fitted, but their uncertainty still has to be reconstructed from the sample size.
Where a review records those standard errors, or records draws that `as_epidist_multivariate()` can turn into a mean vector and a covariance, that uncertainty and the correlation between the parameters reach the model instead of being replaced by a function of the sample size.
...
With the metadata `epireview` records, unadjusted daily date differences from studies flagged as adjusted for truncation, the forward model changes nothing visible.
For a uniform primary event the expected integer date difference equals the continuous mean exactly, and the implied standard deviation only picks up the rounding variance of the two daily windows.
For the Gamma Rosello reported below that moves a standard deviation of 5.41 to 5.43.
So the comparison tests the hierarchical pooling of the `(1 | study)` terms rather than a bias correction.
One of the 29 studies lets us check what that pooling does.
...
isiro_phase <- unique(
  filter(as_tibble(ebola_meta), .data$study == "Rosello 2015 (3)")$phase
)

isiro_draws <- delay_summary_draws(
  fit_ebola,
  newdata = epidist_newdata(
    ebola_meta, study = "Rosello 2015 (3)", phase = isiro_phase
  )
) |>
  ungroup()
```
...
#> 2 meta model, study level      10.71 (8.86, 14.16) 5.25 (3.96, 7.60)
#> 3 meta model, population level 9.35 (8.22, 10.72)  4.84 (3.94, 5.99)
...
density_band <- function(draws, source, n = 200) {
  out <- draws |>
    ...
    # A fresh index for the sampled draws, so this does not depend on the
    # `.draw` column the posterior carries.
...
The Gamma Rosello reported is already within half a day of the re-analysis on the mean.
The study level estimate is shrunk away from it, towards the population mean, by the `(1 | study)` term on the mean.
Its spread is the study's own, because the `(1 | study)` term on the shape lets a study's reported standard deviation inform its own shape rather than sharing one shape across all 29 studies, 15 of which report a single summary with no spread at all.
The between study standard deviation of the shape is large, so that sharing would have misdescribed the Isiro study in particular, whose reported Gamma is much more peaked than the population one.
...
The adjustment the model can make is limited by what the review recorded, and here that is too little for it to make any.
