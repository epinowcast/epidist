...
The meta model supports the following approaches.
...
If you cannot tell which of these a study used, state the assumption you are making alongside any results.
...
Figure \@ref(fig:outbreak) shows the event windows of the simulated cases.
...
(ref:outbreak) The primary and secondary event windows of 200 evenly spaced cases from the simulated outbreak, ordered by primary event time.
...
<details><summary>Click to expand for the study designs and the code to build the table</summary>
...
(ref:studies) The ten simulated studies, their estimation procedures and what each reports.
...
`re_formula = NA` switches the study term off, giving the population level delay, which we compare with the fit without the term.
Figure \@ref(fig:study-term) shows the delay distribution each fit implies.

<details><summary>Click to expand for code to prepare the study term plot</summary>
...
(ref:study-term) The delay distribution implied by the fits with and without a study term. Lines are the posterior median density and bands the 90% interval. The dashed line is the simulated lognormal.
...
<div class="figure">
<img src="figures/meta-study-term-1.png" alt="(ref:study-term)"  />
<p class="caption">(\#fig:study-term)(ref:study-term)</p>
</div>
...
As a reference, we fit the marginal model (`vignette("epidist")`) to the whole, effectively untruncated, line list.
This is what we would recover with access to every underlying delay rather than to published summaries.
...
(ref:compare) Posterior draws of the delay distribution parameters from the five fits, compared to the true values used in the simulation (dashed lines). The bias-adjusted, mixed, and reference fits agree closely with the truth. The fit told that every study had adjusted for right truncation does not.
...
It is not on CRAN, and so we install it from the [mrc-ide r-universe](https://mrc-ide.r-universe.dev).
...
`epidist_estimates_epireview()` maps each record of `onset_to_death` to the long format `as_epidist_estimates_data()` expects.
...
A study reporting a spread that does not match its own value type, such as a median alongside a standard deviation, keeps only the matching set, because the two kinds of summary from a study that reported integer date differences are fitted as though they were independent.
A study that reports the natural parameters of a distribution it fitted is converted as `epidist_estimates_parameters()` would convert it.
The fitted distributions here are Gammas that epireview records as a mean and a standard deviation, which the function picks up as moments.

The metadata we assume for every study is passed as arguments.
Where we know the metadata of individual studies, from the papers or another source, the `metadata` argument takes a table with a row per study that overrides these, and the result is an `epidist_estimates_data` object that can be edited afterwards.
`keep` carries the `phase` covariate onto the rows.
...
The term allows for real heterogeneity across outbreaks and Ebola species, and for studies whose metadata we have got wrong.
...
We assume that studies may differ by about a quarter on the log scale, so both between study standard deviations get `normal(0, 0.25)`.
...
::: {.alert .alert-warning}
These estimates should not be used as they stand.
The estimation approach of every study was assumed rather than checked against the paper.
A publication quality meta analysis would have to do that for each study.
:::
...
## Does any one study drive the estimate? {#leave-one-out}

`epidist_meta_leave_one_out()` refits the model once per study with that study removed.
We compare the population level delay at the post outbreak reference level.


``` r
ebola_leave_one_out <- epidist_meta_leave_one_out(
  fit_ebola,
  newdata = filter(phase_grid, .data$phase == "post outbreak"),
  refresh = ifelse(interactive(), 250, 0)
)
...
`difference` is each refit's posterior relative to the full fit's median.


``` r
ebola_leave_one_out |>
  select("study", "summary", starts_with("difference"))
#> # A tibble: 58 × 5
#>    study            summary difference difference_lower difference_upper
#>    <chr>            <chr>        <dbl>            <dbl>            <dbl>
#>  1 Xu 2016          mean       0.166             -0.989            1.58
#>  2 Xu 2016          sd         0.0646            -0.906            1.17
#>  3 Uyeki 2016       mean       0.00162           -1.01             1.26
#>  4 Uyeki 2016       sd        -0.0231            -0.828            0.995
#>  5 Senga 2016 (1)   mean       0.00901           -1.06             1.25
#>  6 Senga 2016 (1)   sd        -0.00440           -0.873            0.942
#>  7 Schieffelin 2014 mean      -0.0488            -1.14             1.23
#>  8 Schieffelin 2014 sd        -0.0253            -0.929            0.962
#>  9 Nanclares 2016   mean      -0.0180            -1.08             1.19
#> 10 Nanclares 2016   sd         0.0287            -0.825            1.16
#> # ℹ 48 more rows
```


``` r
ggplot(ebola_leave_one_out) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_pointrange(
    aes(
      x = difference, xmin = difference_lower, xmax = difference_upper,
      y = study
    )
  ) +
  facet_wrap(vars(summary), scales = "free_x") +
  labs(x = "Difference from the full fit (days)", y = "Study held out") +
  theme_minimal()
```

<div class="figure">
<img src="figures/meta-leave-one-out-1.png" alt="Posterior median and 95% interval of the delay mean and standard deviation with each study held out, relative to the full fit's median."  />
<p class="caption">(\#fig:leave-one-out)Posterior median and 95% interval of the delay mean and standard deviation with each study held out, relative to the full fit's median.</p>
</div>
