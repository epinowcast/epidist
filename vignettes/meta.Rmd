...
`delays_to_multivariate()` produces the summary vector and its covariance from a set of delays, so it is what a site with a line list it cannot release would run and publish.

::: {.alert .alert-primary}
**What to report, best first.**
The delays themselves, fitted with the marginal model.
Failing that, the parameters of a fitted distribution, or posterior draws of them, with their covariance, through `parameters_to_multivariate()` or `draws_to_multivariate()`.
Failing that, a vector of summaries with a covariance over them, through `delays_to_multivariate()`.
...
Summaries of different kinds from one study, such as a mean and a median, are still treated as independent, which understates their joint uncertainty.
...
simulate_study <- function(study, report, probs, cens_adjusted,
  ...
  if (report == "covariance") {
    return(do.call(delays_to_multivariate, c(
...
simulated_studies <- study_designs |>
  pmap(function(study, report, probs, cens_adjusted, trunc_adjusted,
    ...
    return(simulate_study(
      ...
      trunc_design, delay_min, study_growth_rate, study_n
    ))
  })
...
|adjusted (MVN)  |mean, sd, quantiles + covariance |1: fully adjusted                      |adjusted      |      Inf|         0| 300|
...
The study reporting a covariance matrix over its summaries could not share its delays, so it published the vector of summaries and the covariance between them from `delays_to_multivariate()` instead.
...
#> Intercept           1.79      0.01     1.77     1.82 1.00     1030      703
#> sigma_Intercept    -0.71      0.02    -0.75    -0.67 1.00      854      749
#>
...
#> Regression Coefficients:
#>                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           1.75      0.01     1.72     1.77 1.00      944      586
#> sigma_Intercept    -0.73      0.02    -0.77    -0.68 1.00      720      612
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
#> Regression Coefficients:
#>                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           1.79      0.01     1.76     1.82 1.00      937      618
#> sigma_Intercept    -0.71      0.02    -0.76    -0.67 1.00      859      574
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
Because phase is so rarely recorded, that comes down to a contrast between the studies recorded as post outbreak and every other study, which mixes the two known to be from during an outbreak with the twenty that say nothing.
