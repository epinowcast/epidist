...
knitr::opts_chunk$set(
  fig.path = file.path("figures", "meta-"),
  collapse = TRUE,
  comment = "#>",
  message = TRUE,
  warning = FALSE,
...
The meta model fits directly to these summarised, and potentially biased, published estimates, jointly with any individual level data we do have, by forward modelling what each study's own estimation procedure would have converged to.
...
The meta model differs in adjusting each summary for the biases of the study's own estimation procedure rather than treating it as an unbiased estimate of the continuous delay, and in building on `brms`, so formulas, families, priors and post-processing all carry over.
...
The meta model reuses the likelihood of the marginal model (`vignette("epidist")`), which relies on the [`primarycensored`](https://primarycensored.epinowcast.org/) package [@primarycensored], for individual level rows, adding a forward model of the naive estimator for summary rows, so both kinds of evidence can be combined in one `brms` [@brms] model.
:::

# What we need from a study {#metadata}

Reviews rarely record exactly how a study estimated its delay.
To use a reported summary we need to know roughly how the study handled two common biases, along with the data process it saw.
...
We simulate an outbreak exactly as in `vignette("epidist")`: a stochastic epidemic (Figure \@ref(fig:outbreak) shows the case counts) with a lognormal delay between primary (symptom onset) and secondary (case notification) events, both interval censored to a day.
...
Now imagine several studies each analysing this outbreak at a different point in time, and each reporting only integer date differences with no adjustment for right truncation, that is `cens_adjusted = 0` and `trunc_adjusted = FALSE`.
...
We now build four such studies, at cutoffs of 8, 12, 16 and 25 days, each with a sample of 250 cases.
Two studies report a naive mean and standard deviation and two report only a naive quantile, so the meta model needs to combine both kinds of summary.

<details><summary>Click to expand for code to build the simulated studies</summary>

```{r}
set.seed(2)
study_cutoffs <- c(8, 12, 16, 25)
study_types <- c("moments", "quantile", "moments", "quantile")

naive_study <- function(cutoff, type, study_n = 250) {
  available <- filter(obs_cens, .data$delay_daily + 1 <= cutoff)
  delays <- available |>
    slice_sample(n = min(study_n, nrow(available))) |>
    pull(delay_daily)
  summary_rows <- if (type == "moments") {
    tibble(
      type = c("mean", "sd"),
    ...
    tibble(
      type = "quantile",
      value = stats::quantile(delays, 0.75, names = FALSE),
      p = 0.75
    )
  }
  return(mutate(summary_rows, n = length(delays), relative_obs_time = cutoff))
}

build_study <- function(cutoff, type, i) {
  return(mutate(naive_study(cutoff, type), study = paste0("study_", i)))
}

biased_estimates_df <- pmap(
  list(study_cutoffs, study_types, seq_along(study_cutoffs)), build_study
) |>
  list_rbind() |>
  mutate(trunc_adjusted = FALSE, cens_adjusted = 0, max_delay = 60)
```

</details>

```{r}
biased_estimates <- as_epidist_estimates_data(biased_estimates_df)
biased_estimates
```
...
Even though every study is individually biased, the meta model recovers the true delay distribution parameters (a log mean of `r meanlog` and log standard deviation of `r sdlog`) closely.

## What happens with the wrong metadata

The metadata about how a study adjusted its estimate matters.
To show this, we refit the same four studies but relabel them as if they had already fully adjusted for censoring and truncation, `cens_adjusted = 1` and `trunc_adjusted = TRUE`.

```{r results='hide', message=FALSE}
wrong_flags_estimates <- biased_estimates_df |>
  mutate(trunc_adjusted = TRUE, cens_adjusted = 1) |>
...
The model has no way to know the metadata is wrong, so it takes the naive summaries at face value and converges confidently on badly biased parameters.
This is the same trade-off as for individual level data: the model can only adjust for the biases it is told about.
...
We add 40 individual records, observed up to day 40, alongside the same four published summaries.
...
predicted_parameters <- list(
  ...
  "wrong metadata" = fit_meta_wrong_flags,
  ...
  mutate(model = factor(
    model,
    levels = c(
      "wrong metadata", "adjusted (summary only)", "mixed",
      "reference (marginal)"
    )
  ))
...
(ref:compare) Posterior draws of the delay distribution parameters from the four fits, compared to the true values used in the simulation (dashed lines). The bias-adjusted, mixed, and reference fits agree closely with the truth; the fit given the wrong metadata does not.

```{r compare, fig.cap="(ref:compare)"}
p_compare
```
...
Alongside the delay type we filter to the value types we can model (means and medians), to estimates reported in days with no scaling exponent, and to parameters that are not inverse rates, so that other value types cannot silently slip in as medians.
...
For these data the hygiene filters remove `r nrow(onset_to_death_all) - nrow(onset_to_death)` rows, but they guard against mis-modelling if the underlying database changes.
...
We treat a study as not adjusted for right truncation only if it was conducted during the outbreak (`method_moment_value` of `"Start outbreak"` or `"Mid outbreak"`); all other studies are treated as unaffected by truncation.
Those early phase studies most likely stopped collecting cases at a calendar date rather than following every case for a common length of time, so we use the `"accrual"` truncation design for them, with a uniform accrual rate (`growth_rate = 0`, the default) since epireview does not report an outbreak growth rate either.
...
A small helper maps each row of `onset_to_death` to the long format `as_epidist_estimates_data()` expects: means become `"mean"` rows (with a matching `"sd"` row where a standard deviation, rather than a standard error, is reported), and medians become `"quantile"` rows at `p = 0.5` (with further quantile rows at `p = 0.25` and `p = 0.75` where an interquartile range is reported).
...
::: {.alert .alert-primary}
`max_delay` is set per study above rather than to one shared value, because a grid short relative to a study's own reported spread biases its implied standard deviation downwards; `as_epidist_estimates_data()` warns if this happens.
That grid is only used to compute the implied summaries of studies with `cens_adjusted = 0`; the `r length(unique(ebola_estimates_df$study[ebola_estimates_df$cens_adjusted == 1]))` studies that already report a fitted distribution (`cens_adjusted = 1`) use its closed form continuous moments directly and need no grid at all.
:::
...
Sizing each study's grid to what it needs, rather than a single shared value large enough for the widest study, and using the accrual design for the two early phase studies rather than treating every study as a single cohort, cut sampling (2 chains, 500 post warmup iterations each, timed with cmdstan's own per chain clock so that Stan compilation is not counted) from about 984 seconds to about 324 seconds.
Fitting this model here, including the one-off cost of compiling the Stan program, took `r round(ebola_fit_time[["elapsed"]], 0)` seconds.
...
For every summary we fitted to, we can compare the reported value to what the model implies, both for that study specifically and for the pooled, population level, delay distribution with the `(1 | study)` term switched off.
Doing this for every row at once, grouped by summary type, gives a forest style plot that is both a posterior predictive check, since most reported values should fall within their study level interval, and a synthesis summary, since the pooled interval shows what the meta-analysis implies each study should have seen.
...
forest_data <- ebola_estimates_df |>
  mutate(
...
```{r forest, fig.cap="(ref:forest)", fig.width=10, fig.height=9}
p_forest
```
...
All `r nrow(reported_gamma)` of them come from the same article [@rosello2015ebola], which fitted a Gamma distribution separately to each of several outbreaks in the Democratic Republic of the Congo, so we label them by outbreak rather than by study.
...
(ref:ebola) The population level onset to death delay implied by the meta model fit to `r nrow(onset_to_death)` published summaries (blue lines, 200 posterior draws), compared to the `r nrow(reported_gamma)` Gamma distributions reported by @rosello2015ebola, one for each outbreak in the Democratic Republic of the Congo that it covered (black lines). The means of those fits are themselves among the summaries the model was fitted to.

```{r ebola, fig.cap="(ref:ebola)"}
p_ebola
```
