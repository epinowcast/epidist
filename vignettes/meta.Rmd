...
predicted_parameters <- list(
  ...
  lapply(\(fit) ungroup(add_summaries(delay_parameter_draws(fit)))) |>
  bind_rows(.id = "model") |>
  filter(.data$.row == 1) |>
  mutate(model = factor(
    model,
...
#>               Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sd(Intercept)     0.14      0.04     0.08     0.23 1.01      329      561
#>
#> Regression Coefficients:
#>                     Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept               2.27      0.07     2.13     2.40 1.00      436      547
#> shape_Intercept         0.70      0.04     0.62     0.78 1.00     1179      552
#> phaseduringoutbreak     0.04      0.14    -0.26     0.30 1.00      492      508
#> phaseunrecorded        -0.03      0.08    -0.19     0.13 1.00      475      582
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
# on the result.
population_draws <- delay_parameter_draws(
...
#>   parameter median  q025  q975
#>   <chr>      <dbl> <dbl> <dbl>
#> 1 shape       2.02  1.85  2.18
#> 2 scale       4.83  4.10  5.72
#> 3 mean        9.71  8.43 11.1
#> 4 sd          6.84  5.91  7.87
```
...
#>  during outbreak - post outbreak    0.427 -2.32   3.14
#>  unrecorded - post outbreak        -0.342 -1.94   1.16
#>
#> Term: phase
#> Type: response
```
...
So the comparison tests the hierarchical pooling of the `(1 | study)` term and the shared shape rather than a bias correction.
...
isiro_draws <- delay_parameter_draws(
  fit_ebola,
  newdata = filter(
...
#> 2 meta model, study level      10.73 (9.14, 13.26) 7.58 (6.37, 9.33)
#> 3 meta model, population level 9.71 (8.43, 11.06)  6.84 (5.91, 7.87)
...
The study level estimate is shrunk away from it, towards the population mean, by the `(1 | study)` term, and is more dispersed than either the reported fit or the re-analysis.
That dispersion comes from the model rather than from the study.
`shape ~ 1` gives every study one shared shape, estimated across all 29 studies, 15 of which report a single summary with no spread at all.
