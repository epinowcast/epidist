...
# Skip the rest of this section when epireview is not available
knitr::opts_chunk$set(eval = requireNamespace("epireview", quietly = TRUE))
...
We treat every study as `cens_adjusted = 0`.
...
Estimates that predate the recent literature on these biases rarely adjusted for censoring at all, and rarely say either way, so assuming otherwise would be optimistic.
We apply no truncation correction at all.
Correcting for truncation needs the observation time, and for a study that stopped at a calendar date it also needs the growth rate of primary events over the collection window.
epireview records neither, and `method_moment_value` gives the phase of the outbreak for only nine of the 29 studies here.
We instead adjust for the phase of the outbreak each estimate was made in, so that the residual truncation bias is estimated from the studies rather than assumed.
The fitted `phase` term contrasts the studies recorded as post outbreak against every other study.
Our prior is that estimates made early, while the outbreak was growing, skew towards shorter delays, and that estimates made after the outbreak, or retrospectively, come closer to the unshortened delay.
The sign of the fitted `phasepost` coefficient (the logged ratio of the post outbreak mean to the rest) tells us whether the data support that direction, and with so few studies recorded the term is a coarse adjustment rather than a rigorous one.
Because phase is so rarely recorded, that contrast mixes the two known to be from during an outbreak with the twenty that say nothing.
It absorbs whatever separates those groups, which need not be truncation alone.
...
row_to_estimates <- function(row) {
  ...
  # epireview records a fitted distribution as a mean and an sd.
  reported_sd <- if (identical(spread_type, "Standard Deviation")) {
    spread
  } else if (identical(row$distribution_par2_type, "sd")) {
...
ebola_estimates_df <- onset_to_death |>
  mutate(
    ...
    cens_adjusted = 0,
    phase = ifelse(
      .data$method_moment_value %in% "Post outbreak", "post", "other"
    ),
...
#>               Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> sd(Intercept)     0.14      0.03     0.09     0.21 1.00      334      691
#>
#> Regression Coefficients:
#>                 Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept           2.26      0.04     2.18     2.34 1.01      425      676
#> shape_Intercept     0.54      0.05     0.44     0.64 1.00     1795      807
#> phasepost           0.01      0.08    -0.15     0.17 1.01      490      628
#>
#> Draws were sampled using sample(hmc). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
...
Fitting this model, including the one-off cost of compiling the Stan program, took 186 seconds.
Sizing each study's grid to its own reported spread, rather than using one shared value large enough for the widest study, is what keeps that cost down.

## Checking the fit against the reported estimates

For every row we fitted to, we compare the reported value with what the model implies, both for that study and for the pooled distribution with the `(1 | study)` term switched off.
Each row predicts the first of the summaries a study computed from the same delays, so a study reporting a mean and a standard deviation appears at its mean.
Grouped by summary type, this gives a forest style plot in which most reported values should fall within their study level interval.
...
p_forest <- ggplot(forest_data, aes(y = label)) +
  geom_errorbar(
    aes(
      xmin = reported_lower, xmax = reported_upper,
      colour = "Reported (1.96 se)"
    ),
    orientation = "y", height = 0
  ) +
  geom_point(aes(x = observed, colour = "Reported (1.96 se)"), size = 1.6) +
  geom_pointrange(
    aes(
  ...
  scale_colour_manual(
    breaks = c("Reported (1.96 se)", "Study level", "Population (pooled)"),
    values = c(
      "Reported (1.96 se)" = "black",
      "Study level" = "#56B4E9",
      "Population (pooled)" = "#D55E00"
    )
  ) +
...
::: {.alert .alert-warning}
The adjustment metadata behind this fit is assumed, not reported.
epireview does not record how these studies handled censoring or truncation, so the intervals below show what the model implies under our stated assumptions rather than a corrected estimate of the Ebola onset to death delay.
Treat it as a demonstration of the workflow.
:::
...
#### Pooled population level estimates {#population-estimates}

A single posterior distribution summarises the pooled, population level delay, the one a prediction with the `(1 | study)` term switched off gives.
Reporting only its natural scale mean understates the posterior, so here we show the shape and the scale, the natural mean and the natural standard deviation, each with a median and a 95% credible interval computed from the posterior draws.
...
population_summary <- population_draws |>
  ...
  pivot_longer(dplyr::everything(), names_to = "parameter", values_to = "value") |>
  summarise(
    median = stats::median(.data$value),
    q025 = stats::quantile(.data$value, 0.025),
    q975 = stats::quantile(.data$value, 0.975),
    .by = parameter
  )
...
Some of these estimates also report a fitted Gamma distribution, which epireview records as a mean and a standard deviation.
Those are fitted to above, and converted to a shape and rate here only to draw the density.
They are moments of a fitted distribution rather than summaries of the raw delays, so their sampling error is smaller than the `sd / sqrt(n)` the model assumes when no standard error is reported, and epireview records none for these studies.

Where a study reports the fitted distribution directly, fitting to its shape and scale is the more natural summary of the delay distribution than the mean and standard deviation alone.
epireview does not record the shape and scale, only the mean and standard deviation of the fitted distribution, so we still convert those moments and fit to them above.
The pooled shape and scale come out of the posterior draws we reuse below.
...
Where a comparable estimate from modern methods that account for censoring and right truncation exists (the Isiro Ebola line list of @rosello2015ebola, re analysed in [bdbv-linelist-analysis](https://github.com/epiforecasts/bdbv-linelist-analysis), is the natural candidate), the pooled posterior from an adjusted meta analysis should sit closer to it than the raw published summaries do.
The adjustment here is driven by the `phase` term and by the study level weights, and its residual truncation correction is admittedly crude because epireview records neither the observation times nor how each study handled truncation (see the note above), so the comparison is a demonstration of the workflow rather than a definitive corrected estimate.
Splitting the two serves the workflow: the reported values are what a naive reader sees, the pooled posterior is what an adjusted read of the same summaries points to, and the modern re analysis is the ground truth worth comparing the latter against.
